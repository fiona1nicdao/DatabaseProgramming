...
q3 = listings.find(q3Query).sort("number_of_reviews", pymongo.ASCENDING).limit(5)
q3Headers, q3Results = [
    "_id",
    "name",
    "number_of_reviews",
...
# Question 4
print("Question 4")
q4Query = {"$and": [{"address.country_code": "PT"}, {"bedrooms": {"$gte": 3}}]}
q4Sort = [
    ("review_scores.review_scores_rating", pymongo.DESCENDING),
    ("number_of_reviews", pymongo.DESCENDING),
]
q4 = listings.find(q4Query).sort(q4Sort).limit(5)
q4Headers, q4Results = [
    "_id",
    "name",
    "address.country",
...
q5Sort = [
    ("review_scores.review_scores_checkin", pymongo.DESCENDING),
    ("number_of_reviews", pymongo.DESCENDING),
]
q5 = listings.find(q5Query).sort(q5Sort).limit(5)
q5Headers, q5Results = [
    "_id",
    "name",
    "number_of_reviews",
...
q6Sort = {"$sort": {"total_bedrooms": pymongo.DESCENDING}}
q6 = listings.aggregate([q6Query, q6Group, q6Sort, {"$limit": 5}])
q6Headers, q6Results = ["host_id", "host_name", "total_bedrooms"], []
for result in q6:
    q6Results.append(
        [result["_id"]["host_id"], result["_id"]["host_name"], result["total_bedrooms"]]
    )
